...
const Fleet = () => {
  ...
  // Share selected ship with the sidebar globally
  useEffect(() => {
    if (selectedShip) {
      // Dispatch custom event to notify sidebar of selected ship
      window.dispatchEvent(new CustomEvent('shipSelected', {
        detail: { ship: selectedShip }
      }));
    }
  }, [selectedShip]);
  ...
  const handleScanComplete = (scanType, scanData) => {
    console.log(`${scanType} scan completed:`, scanData);
